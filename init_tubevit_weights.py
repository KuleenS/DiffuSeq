...
from TubeViT.tubevit.model import TubeViT


@click.command()
@click.option("-nc", "--num-classes", type=int, default=2, help="num of classes of dataset.")
@click.option("-f", "--frames-per-clip", type=int, default=200, help="frame per clip.")
@click.option("-v", "--video-size", type=click.Tuple([int, int]), default=(720, 720), help="frame per clip.")
@click.option(
    "-o",
    "--output-path",
    type=click.Path(),
    default="init_tubevit.pt",
    help="output model weight name.",
)
def main(num_classes, frames_per_clip, video_size, output_path):
    x = np.random.random((1, frames_per_clip, video_size[0], video_size[1], 3))
    x = Tensor(x)
    print("x: ", x.shape)
